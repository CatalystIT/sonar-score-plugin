...
public class ScoreDecorator implements Decorator {
 ...
 /**
 * @DependedUpon: The points metric is calculated after the other metrics that is depends upon are calculated first.
 ...
 */
  ...
  */
  ...
  public boolean shouldDecorateResource( final Resource resource, final DecoratorContext context ) {
    //if the resource is not a unit test class, then proceed with decoration/analysis
	  return !ResourceUtils.isUnitTestClass(resource);
  }

  /**
   * Calculates the points metric and saves it to the database.  First, all the necessary measures are retrieved 
   * in order to calculate the points metric for a particular project.
   */
  public void decorate( final Resource resource, final DecoratorContext context ) {
    /*
     * if the resource to decorate is not a unit test class, then retrieve the various code metrics and calculate
     * the points for a given resource/project
     */
